...
import numpy
import scipy.linalg
from firedrake import Function, norm, solve, inner, dx, replace, assemble, dot

def zero_function(y0, length):
   out = []
   for _ in range(length):
      out.append(Function(y0))
      out[-1].assign(0)
   return out
def kiops_fem(τ_out, A, u, tol = 1e-10, m_init = 10, mmin = 10, mmax = 128, iop = 2, task1 = False, params=None):
   """
   ...
   * Niesen, J. and Wright, W.M., 2012. Algorithm 919: A Krylov subspace algorithm for evaluating the ``φ``-functions appearing in exponential integrators. ACM Transactions on Mathematical Software (TOMS), 38(3), p.22
   ...
   if p == 0:
      ...
      u.append(Function(u[0]))
      u[-1].assign(0)
   # We only allow m to vary between mmin and mmax
   m = max(mmin, min(m_init, mmax))

   # Preallocate matrix
   V = zero_function(u[0], mmax+1)#[None for _ in range(mmax + 1)]
   Vj = numpy.zeros((mmax + 1, p))
   ...
   one = Function(u[0])
   one.assign(1)
   local_nrmU = [norm(u[i], 'L1') for i in range(1, len(u))] 
   ...
   u_flip = u[1:][::-1]
   for i in range(len(u_flip)):
      u_flip[i] = Function(u_flip[i])
      u_flip[i] *= nu
   ...
   while τ_now < τ_end:
      ...
      if j == 0:
         ...
         for k in range(p-1):
            i = p - k + 1
            Vj[j, k] = (τ_now**i) / math.factorial(i) * mu
         Vj[j, p-1] = mu

         # Normalize initial vector (this norm is nonzero)
         #local_sum = V[0, 0:n] @ V[0, 0:n]
         local_sum = assemble(dot(V[0], V[0]) * dx)
         #global_sum_nrm = numpy.empty_like(local_sum)
         global_sum_nrm = local_sum
         β = numpy.sqrt( global_sum_nrm + Vj[j, 0:0+p] @ Vj[j, 0:0+p] )
      ...
      while j < m:
         ...
         test_f = A.arguments()[0]
         solve(inner(V[j], test_f) * dx - replace(A, {u[0]: V[j-1]}) == 0, V[j], solver_parameters=params)
         for i in range(p):
            if Vj[j-1, i] != 0:
               V[j] += float(Vj[j-1, i]) * u_flip[i]
         #V[j, 0:n    ] = A( V[j-1, 0:n] ) + V[j-1, n:n+p] @ u_flip
         Vj[j, 0:p-1] = Vj[j-1, 1:p]
         Vj[j, -1     ] = 0.0

         # Classical Gram-Schmidt
         ilow = max(0, j - iop)
         local_sum = []
         for i in range(ilow, j):
            local_sum.append(assemble(dot(V[i], V[j]) * dx))
         #local_sum = V[ilow:j, 0:n] @ V[j, 0:n]
         global_sum = local_sum
         H[ilow:j, j-1] = global_sum + Vj[ilow:j, 0:p] @ Vj[j, 0:p]

         for i in range(ilow, j):
            if H[i, j-1] != 0:
               V[j] -= V[i] * float(H[i, j-1])
         Vj[j, :] = Vj[j, :] - Vj[ilow:j,:].T @ H[ilow:j, j-1]

         local_sum = assemble(dot(V[j], V[j]) * dx)#V[j, 0:n] @ V[j, 0:n]
         #MPI.COMM_WORLD.Allreduce([local_sum, MPI.DOUBLE], [global_sum_nrm, MPI.DOUBLE])
         global_sum_nrm = local_sum
         nrm = numpy.sqrt( global_sum_nrm + Vj[j, 0:p] @ Vj[j, 0:p] )
